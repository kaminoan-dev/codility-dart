 ...
 */
List<int> solution(final List<int> A, final int K) {
  // An empty list has nothing to do.
  if (A.isEmpty || K == 0) {
    return A;
  }

  final int mod = (A.length + K) % A.length;
  ...
  // Splice at modulo and swap the tail and head.
  final int split = A.length - mod;
  final List<int> head = A.sublist(split);
  final List<int> tail = A.sublist(0, split);
